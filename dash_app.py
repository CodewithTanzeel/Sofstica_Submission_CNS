...
app.layout = dbc.Container([
    ...
    dbc.NavbarSimple(
        brand="🛰️ DNS Exfiltration Detector",
        brand_style={"fontWeight": "bold"},
        color="dark", dark=True, fluid=True, className="mb-3",
    ),
    dbc.Tabs([
        dbc.Tab(overview_tab, label="📊 Overview"),
        dbc.Tab(score_tab, label="🔍 Score traffic"),
        dbc.Tab(model_tab, label="🧠 Model internals"),
    ]),
    html.Footer(
...
def run_scoring(n_clicks, sample_file, stateful_contents, stateless_contents):
    try:
        ...
        out = html.Div([
            ...
            dbc.Alert("⚠️ Advisory only — this does not block traffic or take any autonomous action.",
                     color="warning"),
